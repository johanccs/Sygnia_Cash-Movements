...
When the step file contradicts `first_draft.md`, the step file is right and `first_draft.md` should be reconciled to it — not the other way round. Note that `first_draft.md` still cites `planning/one.md` by name in several places; those are historical references to the scaffold step, now `docs/project-scaffold-done.md`.
